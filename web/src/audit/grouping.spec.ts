...
import {
  EXPAND_ALL_BELOW, IMPACT_ORDER, groupByImpact, startsExpanded
} from './grouping'

const violation = (impact: Impact | null, ruleId = 'rule'): Violation => ({
  ruleId, impact, description: 'A description', helpUrl: 'https://example.test', nodes: []
})

describe('groupByImpact', () => {
  it('orders groups most severe first, which is also fix order', () => {
    const groups = groupByImpact([
      violation('minor'), violation('critical'), violation('moderate'), violation('serious')
    ])

    expect(groups.map((group) => group.impact)).toEqual([
  ...
  it('keeps every violation exactly once', () => {
    ...
    const violations = [
      violation('critical', 'a'), violation('critical', 'b'), violation(null, 'c'),
      violation('minor', 'd'), violation('serious', 'e')
    ]
