...
- **A failed enqueue deletes its audit row.** A queued audit nothing will run shows as permanently in progress. An *unconfirmed* enqueue keeps the row — the queue may have taken the job and lost the reply. If the delete fails too, nothing is retried: the worklist ignores unfinished audits older than a twelve-hour grace window, so the page returns on its own rather than being hidden forever by a row nothing will finish.
