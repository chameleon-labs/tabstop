...
} from '../../data/protocols/cryptography/alert-unsubscribe-token-codec.js'
...
export class HmacAlertUnsubscribeToken implements AlertUnsubscribeTokenCodec {
  ...
  decode (token: string): string | null {
    ...
    if (pageId === undefined || signature === undefined ||
        !this.isPageId(pageId) || !SIGNATURE.test(signature)) {
      return null
    }
  ...
  private signatureFor (pageId: string): string {
    return createHmac('sha256', this.secret)
      .update(`tabstop:page-alerts:v1:${pageId}`)
      .digest('base64url')
  }
