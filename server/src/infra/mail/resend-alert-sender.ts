...
const responseName = (body: unknown): string | null =>
  typeof body === 'object' && body !== null && 'name' in body && typeof body.name === 'string' ? body.name : null;
    ...
    if (!response.ok) {
      ...
      if (response.status === 429 && name === 'rate_limit_exceeded') {
        const retryAfterMs =
          ...
          secondsToMilliseconds(response.headers.get('ratelimit-reset')) ??
          MIN_RETRY_AFTER_MS;
        throw new AlertRateLimitError(clampRetryAfter(retryAfterMs));
      }
      if (response.status === 429 && name === 'daily_quota_exceeded') {
        const retryAfterMs = secondsToMilliseconds(response.headers.get('retry-after')) ?? DAILY_QUOTA_RETRY_AFTER_MS;
        throw new AlertRateLimitError(clampRetryAfter(retryAfterMs));
      }
      if (response.status === 429 && name === 'monthly_quota_exceeded') {
        throw new PermanentAlertDeliveryError(`resend:${response.status}:${providerNameForReason(name)}`);
      }
      if (response.status === 429 || (response.status === 409 && name === 'concurrent_idempotent_requests')) {
