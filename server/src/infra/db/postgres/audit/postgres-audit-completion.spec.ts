...
describe('PostgresAuditRepository completion', () => {
  ...
  const storeViolations = async (
    auditId: string, violations: readonly SnapshotViolation[]
  ): Promise<void> => {
    if (violations.length === 0) return

    await db.insertInto('violations').values(violations.map((violation) => ({
      audit_id: auditId,
  ...
  const doneAudit = async (
    ...
    const row = await db.insertInto('audits').values({
      page_id: pageId,
      url: `https://${randomUUID()}.test/a`,
      status: 'done',
      score: params.score,
  ...
  const claimedAudit = async (
    pageId: string | null, createdAt: Date
  ...
  const alertsFor = async (pageId: string) =>
    await db.selectFrom('alert_events').selectAll()
      .where('page_id', '=', pageId).orderBy('id').execute()
  ...
  it('completes a first audit without creating an alert', async () => {
    ...
    const stored = await db.selectFrom('audits').selectAll()
  ...
  it('does not compare against a completed audit created after the current one', async () => {
    ...
    await doneAudit(pageId, {
      score: 10, createdAt: new Date('2026-01-03T10:00:00Z')
    })
    const current = await claimedAudit(pageId, new Date('2026-01-02T10:00:00Z'))
  ...
  it('completes an anonymous audit without looking for an alert', async () => {
    ...
    const stored = await db.selectFrom('audits').select('status')
      .where('id', '=', current.id).executeTakeFirstOrThrow()
    expect(stored.status).toBe('done')
    expect(await db.selectFrom('alert_events').select('id')
      .where('audit_id', '=', current.id).execute()).toEqual([])
  })
  ...
  it('treats a second regression on the same UTC day as a normal no-op', async () => {
    const pageId = await makePage()
    await doneAudit(pageId, {
      score: 100, createdAt: new Date('2026-01-01T10:00:00Z')
    })
    ...
    const stored = await db.selectFrom('audits').select('status')
      .where('id', '=', second.id).executeTakeFirstOrThrow()
    expect(stored.status).toBe('done')
  })
