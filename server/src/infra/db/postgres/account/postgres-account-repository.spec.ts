...
describe('PostgresAccountRepository', () => {
  ...
  describe('add', () => {
    it('creates an account with the default alert threshold', async () => {
      ...
      expect(account).toEqual({
        id: expect.any(String), email, alertThreshold: 5, createdAt: expect.any(Date)
      })
    })
  ...
  describe('loadBySessionId', () => {
    const makeAccountWithSession = async (expiresAt: Date): Promise<{
      account: Awaited<ReturnType<PostgresAccountRepository['add']>>, sessionId: string
    }> => {
