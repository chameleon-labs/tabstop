...
describe('accounts schema', () => {
  ...
  it('defaults alert_threshold to 5 and rejects out-of-range values', async () => {
    ...
    await expect(
      db.insertInto('users').values({
        email: `${randomUUID()}@test.test`, password_digest: 'x', alert_threshold: 0
      }).execute()
  ...
  it('reports the named unique constraint on a duplicate email', async () => {
    ...
    await expect(
      db.insertInto('users').values({ email, password_digest: 'y' }).execute()
  ...
  it('enforces unique (user_id, domain) but allows the same domain for another user', async () => {
    ...
    await expect(
      db.insertInto('sites').values({ user_id: userA, domain }).execute()
    ).rejects.toThrow(/sites_user_domain_unique/)
  ...
  it('cascades a user delete through sites, pages, audits, violations, alerts and sessions', async () => {
    ...
    await db.insertInto('alert_events').values({
      page_id: page.id, audit_id: audit.id, kind: 'score_drop'
    }).execute()
    await db.insertInto('sessions').values({
      id: randomUUID(), user_id: userId, expires_at: new Date(Date.now() + 60_000)
    }).execute()
    ...
    expect(remaining).toEqual({
      sites: 0, pages: 0, audits: 0, violations: 0, alerts: 0, sessions: 0
    })
  })
  ...
  it('excludes an expired session from the auth query', async () => {
    ...
    const live = await db.selectFrom('sessions').selectAll()
      .where('id', '=', liveId).where('expires_at', '>', new Date()).executeTakeFirst()
    const dead = await db.selectFrom('sessions').selectAll()
      .where('id', '=', deadId).where('expires_at', '>', new Date()).executeTakeFirst()
  ...
  it('joins page -> site -> user for the alert threshold #14 reads', async () => {
    ...
    const result = await db.selectFrom('pages')
