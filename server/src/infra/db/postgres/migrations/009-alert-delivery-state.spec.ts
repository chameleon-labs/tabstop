...
describe('009 alert delivery state', () => {
  ...
  it('rejects an alert event that is both emailed and failed', async () => {
    ...
    await expect(insertAlertEvent(pageId, auditId, new Date(), new Date(), 'provider rejected it')).rejects.toThrow(
      /alert_events_delivery_terminal_exclusive/,
    );
  });
  ...
  it('requires a failure reason exactly when an alert event failed', async () => {
    ...
    await expect(insertAlertEvent(pageId, auditId, null, null, 'provider rejected it')).rejects.toThrow(
      /alert_events_failure_reason_pair/,
    );
    await expect(insertAlertEvent(pageId, auditId, null, new Date(), null)).rejects.toThrow(
      /alert_events_failure_reason_pair/,
    );
  });
  ...
  it('refuses a downgrade that would discard preview or terminal state', async () => {
    ...
    `.execute(db);
    await insertAlertEvent(failed.pageId, failed.auditId, null, new Date(), 'resend:451:unavailable_for_legal_reasons');
    ...
    const index = await sql<{indexdef: string}>`
      ...
      where schemaname = 'public'
        and indexname = 'alert_events_unsent_idx'
    `.execute(db);
    expect(index.rows[0]?.indexdef).toContain('WHERE ((emailed_at IS NULL) AND (failed_at IS NULL))');
  });
});
