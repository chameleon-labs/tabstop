...
describe('schema deletion semantics', () => {
  ...
  it("deletes a page's audits, violations and alert events", async () => {
    ...
    const alerts = await db.selectFrom('alert_events').select('id').where('page_id', '=', fixture.pageId).execute();
