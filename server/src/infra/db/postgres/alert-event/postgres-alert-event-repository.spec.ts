...
describe('PostgresAlertEventRepository', () => {
  ...
  const fixture = async () => {
    ...
    const current = await db.insertInto('audits').values({
      ...
      score: 72,
      axe_version: '4.12.1'
    }).returning(['id', 'public_uuid']).executeTakeFirstOrThrow()
    ...
    const event = await db.insertInto('alert_events').values({
      page_id: page.id,
      audit_id: current.id,
  ...
  it('marks delivery once and never rewrites its first confirmed time', async () => {
    ...
    const stored = await db.selectFrom('alert_events').select('emailed_at')
      .where('id', '=', event.id).executeTakeFirstOrThrow()
  ...
  it('disables only mail alerts and leaves daily monitoring enabled', async () => {
    ...
    const stored = await db.selectFrom('pages')
