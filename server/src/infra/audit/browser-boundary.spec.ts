...
const compileUnder = async (configPath: string, source: string): Promise<string> => {
  ...
  try {
    ...
    const { stdout } = await run(TSC, ['-p', join(directory, 'tsconfig.json')])
...
const compileInBrowserProject = async (source: string): Promise<string> => {
  ...
  try {
    ...
    const { stdout } = await run(TSC, ['-p', config])
      .catch((error: { stdout?: string }) => ({ stdout: error.stdout ?? '' }))
    return stdout
  } finally {
...
describe('browser compilation boundary', () => {
  it('compiles the browser unit with DOM types available', async () => {
    ...
    const output = await compileUnder(
      BROWSER_CONFIG,
  ...
  it.each(['document', 'window', 'localStorage'])(
    'still refuses %s in the main server program',
    async (global) => {
      ...
      const output = await compileUnder(
        MAIN_CONFIG,
        `export const probe = (): unknown => ${global}\n`
      )
