...
import {
  ...
  mockAuditModel,
  mockLoadAuditByIdRepository,
  mockPageAuditor,
  mockReplaceViolationsRepository,
} from '../../test/index.js';
...
describe('DbRunAudit', () => {
  it('claims, stores violations, then completes with the rule snapshot', async () => {
    ...
    expect(violations.replaceAll.mock.invocationCallOrder[0]).toBeLessThan(
      auditStatus.complete.mock.invocationCallOrder[0] ?? 0,
    );
  ...
  it('marks failed and raises PermanentAuditError for a permanent failure', async () => {
    ...
    expect(auditStatus.markFailed).toHaveBeenCalledWith(
      'audit-1',
  ...
  it('hands the claim back when a transient failure will be retried', async () => {
    ...
    expect(auditStatus.releaseClaim).toHaveBeenCalledWith('audit-1', new Date('2026-07-27T10:00:00Z'));
  });
  ...
  it('acknowledges an unclaimable audit that has already finished', async () => {
    for (const status of ['done', 'failed'] as const) {
      ...
      loadAudit.loadById.mockResolvedValueOnce(mockAuditModel()).mockResolvedValueOnce({...mockAuditModel(), status});
