...
describe('DbUnsubscribePageAlerts', () => {
  it('disables alerts for the authenticated page token', async () => {
    ...
    await expect(new DbUnsubscribePageAlerts(tokens, pages).unsubscribe('token')).resolves.toBe(true);
  ...
  it('does not touch storage for a tampered token', async () => {
    ...
    await expect(new DbUnsubscribePageAlerts(tokens, pages).unsubscribe('tampered')).resolves.toBe(false);
