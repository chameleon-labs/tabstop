...
import type {LoadPendingAlertEventsRepository} from '../../protocols/db/alert-event/load-pending-alert-events-repository.js';
...
describe('DbDispatchPendingAlertEmails', () => {
  it('reports processed candidates without claiming each created queue work', async () => {
    const alerts: LoadPendingAlertEventsRepository = {
      loadPendingAlertEventIds: vi.fn().mockResolvedValueOnce(['1', '2']).mockResolvedValueOnce(['3']),
    };
    ...
    expect(alerts.loadPendingAlertEventIds).toHaveBeenNthCalledWith(1, null, 2, 'delivery');
    expect(alerts.loadPendingAlertEventIds).toHaveBeenNthCalledWith(2, '2', 2, 'delivery');
  ...
  it('propagates an enqueue failure so BullMQ retries the dispatch', async () => {
    ...
    await expect(new DbDispatchPendingAlertEmails(alerts, queue).dispatch()).rejects.toThrow('redis unavailable');
  });
  ...
  it('keeps preview mode on every keyset page', async () => {
    ...
    expect(alerts.loadPendingAlertEventIds).toHaveBeenNthCalledWith(1, null, 2, 'preview');
    expect(alerts.loadPendingAlertEventIds).toHaveBeenNthCalledWith(2, '2', 2, 'preview');
  });
});
