...
const worsenedViolations = (delivery: AlertDelivery): WorsenedViolation[] => {
  ...
  const previousByRule = new Map(
    delivery.previous.violations.map((violation) => [violation.ruleId, violation])
  )
  ...
  return [
    ...
    ...diff.unchanged.flatMap((current) => {
      ...
      const impactWorsened = current.impact !== null &&
        (previous.impact === null || impactRank[current.impact] > impactRank[previous.impact])
      return impactWorsened || current.nodeCount > previous.nodeCount
        ? [{ current, previous }]
        : []
    })
  ]
...
const violationLine = ({ current, previous }: WorsenedViolation): string => {
  ...
  const impact = previous !== null && previous.impact !== current.impact
    ? `${previous.impact ?? 'unknown'} → ${currentImpact}`
    : currentImpact
  const nodes = previous !== null && previous.nodeCount !== current.nodeCount
...
const renderAlertEmail = (
  ...
  const unsubscribeUrl = new URL(
    `/api/alerts/unsubscribe/${unsubscribeToken}`, publicApiOrigin
  ).toString()
