...
export interface AuditJobQueue {
  ...
  /**
   * How many jobs are waiting to be picked up. Submission needs it because a
   * per-requester rate limit bounds one source and the queue is shared by all
   * of them: enough distinct sources, each politely inside its own allowance,
   * still drive the backlog to any length they collectively want.
   */
