...
import {
  detectRegression, diffViolations, type AuditSnapshot, type ViolationSnapshot
} from './regression.js'

const violation = (
  ruleId: string, impact: ViolationSnapshot['impact']
): ViolationSnapshot => ({ ruleId, impact })
...
describe('detectRegression', () => {
  const cases: Array<{
    ...
    },
    {
      ...
      current: snapshot({
        violations: [
          violation('existing', 'minor'),
          violation('focus-order-semantics', 'serious')
        ]
      }),
