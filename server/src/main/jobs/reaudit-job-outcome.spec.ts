...
describe('reauditRunFailure', () => {
  ...
  it('fails a run that could not schedule some pages', () => {
    expect(reauditRunFailure(summary({ failed: 3 }), false))
      .toContain('could not schedule 3 page(s)')
  })
  ...
  it('fails a run that stopped with pages still due', () => {
    ...
    expect(reauditRunFailure(summary({ truncated: true, pagesConsidered: 50_000 }), false))
      .toContain('stopped at 50000 pages')
  })
  ...
  it('says which problem it is, so the failed job is worth reading', () => {
    ...
    expect(reauditRunFailure(summary({ failed: 2, truncated: true }), false))
      .toContain('could not schedule 2 page(s)')
  })
  ...
  it('reports success for a night with nothing to do', () => {
    ...
    expect(reauditRunFailure(summary({ pagesConsidered: 0, auditsEnqueued: 0 }), false))
      .toBeNull()
  })
})
