...
const logSend = (alertEventId: string, outcome: SendAlertEmailOutcome, attempt: number): void => {
  console.log(
    JSON.stringify({
...
export const makeAlertEmailJobProcessor =
  ({rateLimit, dispatch, send}: AlertEmailJobProcessorDependencies) =>
  async (job: AlertEmailJob): Promise<void> => {
