...
import {AlertRateLimitError, type AlertSender} from '../../data/protocols/mail/alert-sender.js';
...
describe('alert email delivery pipeline', () => {
  ...
  const start = async (
    ...
    worker = makeWorker<AlertQueuePayload>(
      queue.name,
      redisUrl,
      makeAlertEmailJobProcessor({
        ...
        dispatch: async () =>
          await new DbDispatchPendingAlertEmails(
            dispatchAlerts,
    ...
    return new DbDispatchPendingAlertEmails(dispatchAlerts, new BullMqAlertEmailQueue(queue), 100, mode);
  };
  ...
  const makePreviewSender = (sender: AlertSender): DbSendAlertEmail =>
  ...
  it('sends one message when overlapping dispatch runs see the same event', async () => {
    ...
    expect(await db.selectFrom('alert_events').select('id').where('id', '=', alertEventId).execute()).toHaveLength(1);
  });

  it('retries provider failure against the same event and marks only the accepted attempt', async () => {
    const alertEventId = await seedEvent();
    const sender: AlertSender = {
      send: vi.fn().mockRejectedValueOnce(new Error('provider unavailable')).mockResolvedValue('accepted'),
    };
    const dispatch = await start(sender, alertEventId);
  ...
  it('pauses provider traffic without consuming a normal job attempt', async () => {
    const alertEventId = await seedEvent();
    const sender: AlertSender = {
      send: vi.fn().mockRejectedValueOnce(new AlertRateLimitError(2_000)).mockResolvedValue('accepted'),
    };
    const dispatch = await start(sender, alertEventId);
    ...
    await eventually(async () => {
      expect(sender.send).toHaveBeenCalledOnce();
      expect(await queue.getRateLimitTtl(ALERT_EMAIL_WORKER_LIMITER.max)).toBeGreaterThan(1_250);
    });
  ...
  it('does not preview twice after completed-job cleanup, and delivery mode still sends once for the seeded event', async () => {
    ...
    const redeliveredPreviewJob = await queue.add(
      'send',
      {
        kind: 'send',
        alertEventId,
