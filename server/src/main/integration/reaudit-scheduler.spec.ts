...
describe('daily re-audit scheduler', () => {
  ...
  it('spreads several pages on one domain rather than firing them together', async () => {
    ...
    const second = await db.insertInto('pages')
      .values({
        site_id: (await db.selectFrom('pages').select('site_id')
          .where('id', '=', first.pageId).executeTakeFirstOrThrow()).site_id,
    ...
    const delays = await Promise.all([first.pageId, second.id].map(async (pageId) => {
      const audits = await auditsFor(pageId)
      const job = await queue.getJob(`audit-${audits[0]?.id ?? ''}`)
