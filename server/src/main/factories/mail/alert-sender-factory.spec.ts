...
describe('makeAlertSender', () => {
  it('uses the non-delivering console adapter unless Resend is explicitly selected', () => {
    expect(makeAlertSender({ mailDriver: 'console', resendApiKey: null }))
      .toBeInstanceOf(ConsoleAlertSender)
  })

  it('builds Resend only when both the driver and key are present', () => {
    expect(makeAlertSender({ mailDriver: 'resend', resendApiKey: 're_test' }))
      .toBeInstanceOf(ResendAlertSender)
    expect(() => makeAlertSender({ mailDriver: 'resend', resendApiKey: null }))
