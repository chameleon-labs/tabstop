...
**The workspace has costs, and they were paid rather than deferred.** `pnpm-workspace.yaml` and the lockfile moved to the root, `server/`'s `packageManager` field was dropped so there is one declaration of it, and Server CI's install now covers `web/`'s dependencies too. That last one is deliberate, though **not for the reason first recorded here.** This entry originally claimed `--filter` would install a different tree from the one a developer has; #18 tested that by running both installs into fresh clones and diffing, and the trees are identical — pnpm gives each package a `node_modules` holding only its own declared dependencies, so filtering changes nothing about what either half can resolve. The real justification is narrower: a full `--frozen-lockfile` install validates the lockfile against *every* project, so a lockfile left stale for one half fails in whichever workflow runs first rather than only in the other one. The path filter gained `contract/**`, because a change there can break the server's build while touching nothing under `server/`.
